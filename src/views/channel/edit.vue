          ...
          :rules="item.required?fieldRequied:[]"   
            ...
            > </cms-upload>
              ...
              <el-select class="cms-width" v-model="info['attr_'+item.field]" 
                ...
                <!-- 工作流 -->
                    <el-select  class="cms-width" v-model="info['workflowId']"
...
export default {
  ...
  methods: {
    ...
    getAllList() {
      ...
      axios
        .all([
          axios.post(api.fullTextSearchChannelList, {
            hasContentOnly: false,
            excludeId: this.params.id
          }), //栏目列表
          ...
          axios.post(api.workflowList, { pageSize: 0, pageNo: 99 }),
    ...
    update(state) {
      ...
      form.validate(valid => {
        ...
        if (valid) {
          this.loading = true;
          axios
            .post(this.$api.channelUpdate, this.getParams())
            .then(res => {
              this.loading = false;
              if (res.code == "200") {
