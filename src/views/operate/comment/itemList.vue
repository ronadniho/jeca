<template>
  ...
  <section v-loading="loading" class="cms-body">
    ...
    <div class="cms-list-header">
      <el-row>
        <el-col :span="12">
           <!-- <el-button type="primary" @click="routerLink('/comment/list',0)" >返回</el-button>     -->
      ...
      <header class="header" >
          <el-row>
            <el-col :span="8" class="title">
              ...
              <a :href="info.contentURL" target="_blank">{{info.contentTitle}}</a>
            </el-col>
            <el-col :span="2">
               <span class="contentId">文章ID：{{info.contentId}}</span>
            </el-col>
          </el-row>
      </header>
        ...
        <footer class="footer">
          <el-row>
            <el-col :span="12" class="gray">
              <font class="nbsp">&nbsp;</font>
              <span>评论者：{{tableData[index].commenterUsername}}</span>
...
export default {
  ...
  data(){
    return {
      ...
      params:{
        queryChecked:'',
        queryContentId:this.$route.query.id,
        queryRecommend:'',
        pageNo: 1,
        pageSize:10,
      },
  ...
  methods:{
    ...
    clickComment(index){
      ...
      obj.commentDisabled = true;
      obj.commentDisplayDisabled = false;
    ...
    cancelComment(index){
      let obj = this.tableData[index];
      obj.commentDisabled = false;
      obj.commentDisplayDisabled = true;
      this.$set(this.tableData,index,obj);
    },
    //修改评论中提交按钮点击事件
    submitComment(index,commentId){
      ...
      axios.post(this.$api.commentUpdate,param).then(res=>{
        ...
        if(res.code == '200'){
          ...
          obj.text = this.commentContent[index];
    ...
    getTableData(){
      this.loading = true;
      axios.post(this.$api.commentItemList,this.params).then(res=>{
        this.loading = false;
        if(res.code == '200'){
          this.tableData = [];
          this.checks = [];
