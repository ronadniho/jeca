    ...
    <section v-loading="loading" class="cms-body">
        ...
        <el-form v-show="next" ref="form" :model="dataInfo" :rules="rules" class="cms-form" label-width="162px">
         ...
         <div v-show="!next" class="com-box"> 
            ...
            <div class="com-right">
                  <h1 class="com-view">表单预览 <span>(可上下拖动组件调整显示顺序,双击修改表单信息)</span> </h1>
                  <el-form  ref="form1" class="cms-form" label-width="162px" style="margin:0">                
                  <ul style="width:100%">
                   <li v-for="(item,index) in filedsArray"  v-on:dblclick="edit(item.id,index)"
                    v-dragging="{ item: item, list: filedsArray, group: 'item'}" :key="index" 
                    class="drag-items"
                    >        
           ...
           <el-form-item label="字段"  prop="field">
            ...
            <el-form-item label="默认值"  prop="defValue">
             ...
             <el-form-item label="可选项:" class="flex-100"  prop="optValue" >
               <el-input  v-model="saveForm.optValue" ></el-input> 
...
export default {
  ...
  methods: {
    ...
    if(val==1){
      ...
      this.loading = true;
      axios
        .post(this.$api.customFiledGet, { id: 0 })
        .then(res => {
    ...
    ok() {
      ...
      form.validate(valid => {
        ...
        if (valid) {
          ...
          axios
            .post(this.$api.customFiledSave, this.saveForm)
            .then(res => {
              if (res.code == "200") {
                let obj = {
                  field: "",
                ...
                obj.id = res.body.id;
                if (
                  this.saveForm.dataType == 7 ||
                  this.saveForm.dataType == 8 ||
                  this.saveForm.dataType == 6
                ) {
    ...
    deleteIndex(index, id) {
      this.loading = true;
      axios
        .post(this.$api.customFiledDelete, { ids: id })
        .then(res => {
          if (res.code == "200") {
    ...
    nextStep() {
      ...
      form.validate(valid => {
        ...
        if (valid) {
          axios
            .post(this.$api.customFormSave, this.dataInfo)
            .then(res => {
              this.customId = res.body.id;
              this.next = false;
              this.loading = false;
            })
            .catch(err => {
    ...
    publish() {
      ...
      if(ids.length>0){
        axios
          .post(this.$api.customFiledPriority, {
            ids: ids,
    ...
    edit(val, index) {
      ...
      this.editObj = "";
      axios
        .post(this.$api.customFiledGet, { id: val })
        .then(res => {
    ...
    update() {
      ...
      form.validate(valid => {
        ...
        if (valid) {
          ...
          axios
            .post(this.$api.customFiledUpdate, this.editObj)
            .then(res => {
              this.loading = false;
              if (res.code == "200") {
